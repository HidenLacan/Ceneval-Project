...
def get_map_url_from_configuracion(configuracion_ruta, verify_exists=True):
    ...
    try:
        if not configuracion_ruta.mapa_html:
            print("No hay mapa_html en la configuración")
            return None
        ...
        if configuracion_ruta.mapa_html.strip().startswith('<!DOCTYPE html>'):
            ...
            print(f"✅ Usando ID de colonia directamente: {colonia_id}")
            ...
            if not match:
                ...
                for pattern in alt_patterns:
                    ...
                    if match:
                        print(f"ID encontrado con patrón alternativo: {pattern}")
                        break
                
                if not match:
                    print(f"No se encontró ID de mapa en mapa_html")
                    print(f" Contenido de mapa_html: {configuracion_ruta.mapa_html[:200]}...")
                    return None
            
            map_id = match.group(1)
        print(f"ID de mapa extraído: {map_id}")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if map_filename in available_maps:
                        print(f"✅ Mapa encontrado en API: {map_filename}")
                    else:
                        print(f"⚠️ Mapa no encontrado en API: {map_filename}")
                        print(f"📁 Mapas disponibles: {available_maps}")
                        return None
                else:
                    print(f"⚠️ No se pudo verificar mapas disponibles: {response.status_code}")
                    
            except Exception as e:
                print(f"⚠️ Error verificando mapas disponibles: {str(e)}")
                # Continue anyway, maybe the API is not available
        
        print(f"🗺️ URL del mapa: {map_url}")
        return map_url
...
def send_route_email(configuracion_ruta, empleados_info):
    ...
    try:
        ...
        if url_map:
            plain_message += f"""
        🗺️ Mapa de la colonia: {settings.BASE_URL}{url_map}
        ...
        if settings.DEBUG or settings.DEFAULT_FROM_EMAIL == 'your-email@gmail.com':
            ...
            print(f"📧 [DEV MODE] Email would be sent to:")
            for empleado in empleados_info:
...
def send_route_email_from_staff_dashboard(colonia_id, employee_ids):
    ...
    try:
        ...
        if not empleados.exists():
            return False, "No se encontraron empleados válidos"
